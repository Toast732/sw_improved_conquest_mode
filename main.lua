	...
	RULES = {
		LOGISTICS = { --? for the logistics/cargo AI
			...
			CONVOY = { --? convoy specific rules
				...
				heli = { --? rules specific for helis
					min_distance = 55, -- make 	 one behind stop moving so it gives it space
					target_distance = 150, -- + spawning distance of both vehicles
		...
		SETTINGS = { --? for the settings command
			...
			CONVOY_FREQUENCY = {
				...
				max = {
					value = 180, --3 hours
					message = "This seens abit long, the AI wont be able to get enough supplys!"
				},
				input_multiplier = time.minute
			},
		}
...
function onCreate(is_world_create, do_as_i_say, peer_id)
	...
	if not g_savedata.settings then
		g_savedata.settings = {
			...
			AI_INITIAL_SPAWN_COUNT = property.slider("AI Initial Spawn Count (Per every second island)", 0, 30, 1, 5),
			...
			CONVOY_FREQUENCY = property.slider("Cargo Convoy Frquency (Mins)", 1, 60, 1, 30) * 60 * 60,
		}
	end
